...
export const linkFile = (path: string, dest: string) => {
  try {
    ...
    if (err.code === "EEXIST") return dest // Return the dest so that we can log a nice error message
    else logError("Unexpected error while linking file " + green(dest), err)
  }
}
