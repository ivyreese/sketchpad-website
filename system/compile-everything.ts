...
import { basename, copyPage, exists, glob, linkFile, read, rm, stats, write, writePage } from "./io.ts"
...
import { compact, compare, replace, unique } from "./util.ts"
...
export const compileEverything = () => {
  ...
  // Now we take everything that's not a page or page template and hardlink it into the public folder.
  // We use hardlinks, rather than copying, because it's much faster.
  // We use path.replace(), rather than the replace({}) helper, because there can be subfolders named (eg) "content"
  let conflicts = [
    glob("content/**/*.!(md|html)").map((path) => linkFile(path, path.replace("content/", "public/"))),
    glob("template/**/*.!(html)").map((path) => linkFile(path, path.replace("template/", "public/"))),
  ].flat()

  // If there were any conflicts when linking the above files, log them out with a nice error message.
  compact(unique(conflicts)).forEach((dest) => {
    const name = dest.replace("public/", "")
    let conflicting = ["content/", "template/"].map((p) => p + name).filter(exists)
    log(red("The following files are conflicting:"))
    logIndented(green(conflicting.join("\n")))
  })
