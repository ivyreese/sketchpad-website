...
const expander = (macro: string, spaces: string, page: Page, pages: Page[]) => {
  ...
  switch (macro) {
    ...
    // TODO: should refactor to include the <figure>
    case macro.startsWith("youtube") && macro: {
      let id = stripName(macro, "youtube")
      return `${spaces}<iframe class="youtube" src="https://www.youtube-nocookie.com/embed/${id}" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>`
    }

    // {{video:foo.mp4}}
    // TODO: add support for alt text
    case macro.startsWith("video") && macro: {
      const src = stripName(macro, "video")
      return `${spaces}<video controls loop muted playsinline preload src="${src}"></video>`
    }
    ...
    // Generate the table of contents for an essay
    case "toc":
      return toc(compiledBody)

    case "href":
      return page.url.pathname

    case "head-title": {
      let title = frontmatter.title || Env.name
...
const toc = (body: string) => {
  // Get all h2 and h3 elements in the essay
  // We're going to process them each in order,
  // building up a tree structure and then converting it to HTML
  const headings = Array.from(body.matchAll(/<h[2-3].*?>(.+?)<\/h[2-3]>/gis))

  if (!headings.length) return ""

  // The TOC is made up of <div> sections, each containing an <h3> and optional <ul>
  // We'll use this array to build up that structure, eg: [[h3, ul], [h3], [h3], [h3, ul]]
  const sections: string[][] = []

  // And we'll use this array to collect items for the current ul
  let list: string[] | null = null
  let isNumbered = false

  // At the end of each section, we convert the current list (if any) to a <ul> string
  const finishCurrentList = () => {
    if (list) {
      const tag = isNumbered ? "ol" : "ul"
      sections.at(-1)!.push(`<${tag}>${flatJoin(list)}</${tag}>`)
      list = null
    }
  }

  // Now, process each h2 and h3 we found in the essay
  for (let [match, content] of headings) {
    if (match.startsWith("<h2")) {
      finishCurrentList()
      sections.push([])
      sections.at(-1)!.push(`<h3>${content}</h3>`)
    } else {
      if (!list) {
        isNumbered = content.startsWith("1")
        list = []
      }
      content = content.replace(/^\d+\.\s*/, "") // Remove section numbering, will use <ol> instead
  ...
  return flatJoin(['<nav id="table_of_contents">', `<h2>Contents</h2>`, sections.map((v) => `<div>${flatJoin(v)}</div>`), `</nav>`])
}
